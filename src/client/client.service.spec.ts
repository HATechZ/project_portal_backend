...
const client = {
  id: 'client-id', companyId: 'company-id', name: 'Client', isActive: true,
  createdAt: new Date(), updatedAt: new Date(),
};
const contact = {
  id: 'contact-id', userId: 'user-id', clientId: client.id, name: 'Contact',
  email: 'contact@example.com', designation: null, phone: null, isPrimary: true,
  isActive: true, createdAt: new Date(), updatedAt: new Date(),
};

describe('ClientService onboarding', () => {
  const clients = {
    findScopedCompany: jest.fn(), findById: jest.fn(), findAll: jest.fn(), count: jest.fn(),
    update: jest.fn(), setActive: jest.fn(),
  };
  const contacts = {
    create: jest.fn(), findById: jest.fn(), findAll: jest.fn(), count: jest.fn(),
    update: jest.fn(), deactivate: jest.fn(), reactivate: jest.fn(), setPrimary: jest.fn(),
  };
  ...
  it('creates Client and Primary Contact without password setup when portal access is disabled', async () => {
    onboarding.create.mockResolvedValue({ client, contact: { ...contact, userId: null }, setupRequired: false });
    ...
    expect(onboarding.create).toHaveBeenCalledWith('company-id', expect.objectContaining({ assignedByUserId: 'admin-id' }));
  ...
  it('initiates the approved setup flow without exposing token material when portal access is enabled', async () => {
    onboarding.create.mockResolvedValue({ client, contact, setupRequired: true });
  ...
  it('uses the same setup port for a portal grant only when credentials still need setup', async () => {
    access.grant.mockResolvedValue({
      ...
      clientContact: { id: contact.id, name: contact.name, email: contact.email, userId: contact.userId },
      setupRequired: true,
    });

    await service.grantPortalAccess(client.id, contact.id, 'admin-id');
  ...
  it('rejects a non-active contact when the primary repository cannot select it', async () => {
    ...
    await expect(service.setPrimary(client.id, contact.id)).rejects.toMatchObject({
