...
import {
  MarketingDocumentCodeRepository,
  MarketingDocumentCodeStatus,
} from '../repositories/marketing-document-code.repository';

export abstract class MarketingDocumentCodeServiceBase {
  protected constructor(
    private readonly codes: MarketingDocumentCodeRepository,
  ) {}

  findAll(status: MarketingDocumentCodeStatus = 'active') {
  ...
  private async setActive(id: string, isActive: boolean) {
    ...
    if (current.isActive === isActive) {
      throw new AppException({
        ...
        message: isActive
          ? 'Marketing Document Code is already active.'
          : 'Marketing Document Code is already inactive.',
      });
    }
  ...
  private async requireCode(id: string) {
    ...
    throw new AppException({
      ...
      message: 'Marketing Document Code was not found.',
    });
  }
  ...
  private async assertUnique(code: string, exceptId?: string) {
    ...
    throw new AppException({
      code: AppErrorCode.Conflict,
      status: HttpStatus.CONFLICT,
      message: 'A Marketing Document Code with the same code already exists.',
    });
  }
}
