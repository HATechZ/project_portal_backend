...
export type MarketingDocumentCodeRecord = Prisma.DocumentCodeOptionGetPayload<{
  select: typeof generalDocumentCodeSelect;
}>;

export type MarketingDocumentCodeStatus = 'active' | 'inactive' | 'all';

@Injectable()
export class MarketingDocumentCodeRepository extends BaseRepository {
  ...
  list(
    status: MarketingDocumentCodeStatus,
  ): Promise<MarketingDocumentCodeRecord[]> {
  ...
  find(id: string): Promise<MarketingDocumentCodeRecord | null> {
    const tenantId = RequestContext.requireTenantId();
    return this.transaction((db) =>
