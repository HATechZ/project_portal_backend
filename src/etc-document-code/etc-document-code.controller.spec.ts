...
describe('EtcDocumentCodeController', () => {
  it('declares the ETC route, authorization and lifecycle without delete', () => {
    const prototype = EtcDocumentCodeController.prototype;
    ...
    expect(Reflect.getMetadata(PATH_METADATA, EtcDocumentCodeController)).toBe(
      'etc-document-codes',
    );
    expect(
      Reflect.getMetadata(PERMISSIONS_KEY, EtcDocumentCodeController),
    ).toEqual([WorkflowActionCode.MANAGE_GENERAL_DOCUMENT_CODES]);
    expect(
      Reflect.getMetadata(GUARDS_METADATA, EtcDocumentCodeController),
