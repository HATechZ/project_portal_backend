...
**Status:** Gate 4 · implementation WIP · 7/10 implemented  
**Spec Reference:** `SPEC.md` · **Plan Reference:** `plan.md`

Every leaf is unticked until an independent verifier runs its static assertion; runtime evidence
is separately recorded in `walkthrough.md`. Implement in this module before `04.2-member`.
...
- [x] **Phase 3: authorization and platform conformance**
  - [x] Apply the established guard chain and `ADD_DIVISION`; depend on mutation routes, require configured permission and same-Company system_admin without wildcard bypass, and deny division_lead/contextual Team Lead. Test 401, 403, own-Tenant allow, foreign-Tenant 404; complete when guard and authorization tests pass.
...
- [ ] **Phase 4: independent verification and sign-off**
  ...
  - [ ] Execute and record the production HTTP/RLS walkthrough; depend on tests, use app_user with isolated cleanup, cover all five routes, 400/401/403/404/409, two-Tenant isolation, envelope, and x-request-id. Complete only with PASS evidence and cleanup.
