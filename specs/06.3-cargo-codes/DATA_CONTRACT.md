...
`BidDetail.cargoCodeOptionId` is nullable `ON DELETE SET NULL`; it contains `cargoName` but no Cargo-code snapshot. No normal delete is safe; identity edit after reference is owner-decided. Existing `option_values` RLS is tenant-scoped. Migration history does not enable RLS on global `option_types`; runtime must safely read fixed Cargo Code catalogue entries and not create arbitrary types. OptionType RLS is optional future security hardening, not a current gap.
