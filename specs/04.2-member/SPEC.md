...
a linked login User; Team and role assignment remain separate.
...
- `[AC-W02]` IF onboarding fails after any intermediate step, THEN the transaction SHALL roll back
  and leave no incomplete Member/User state.
...
- `[AC-W05]` IF a `division_head`, `division_lead`, or `team_lead` attempts to create a Member outside their
  ...
  creating any row.
- `[AC-W06]` IF a caller supplies `roleId`, THEN `POST /member` SHALL reject the unknown field
  without creating partial onboarding state.
...
ActorProfile activation remain module 03 capabilities. `POST /member` creates only linked User
and Member records and must not duplicate access-assignment logic or invent
