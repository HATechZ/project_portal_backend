...
- [x] **Phase 1: Member CRUD foundation**
  ...
  - [x] Implement V1 onboarding create/update DTO allow-lists; depend on scoped reads, derive Tenant/Company, validate actor-scoped Division and Tenant-unique business email, require password but reject `roleId` on create, expose designation without a role in responses, reject forbidden ownership/User/ActorProfile/hash/Team fields, create linked User and Member without UserRole/ActorProfile/Team assignment, and block a Division move that conflicts with active Team lead/membership. Test unassigned onboarding, no persistence-level Team assignment, safe move, conflict move, and invalid input; complete when DTO/service tests pass.
        VERIFY: grep -q "password" src/member/dtos/create-member.dto.ts && ! grep -q "roleId" src/member/dtos/create-member.dto.ts && ! grep -qE "tenantId|companyId|userId|actorProfileId|passwordHash|confirmPassword|teamId" src/member/dtos/create-member.dto.ts && ! grep -qE "userId|password|roleId|actorProfileId|teamId|tenantId|companyId" src/member/dtos/update-member.dto.ts && grep -q "designation" src/member/dtos/member-response.dto.ts && ! grep -qE "roleId|division\?|user\?" src/member/dtos/member-response.dto.ts && ! grep -qE "userRole\.|actorProfile\.|db\.role\." src/member/repositories/member-onboarding.repository.ts && corepack yarn test --runInBand --testPathPatterns=member
  - [x] Implement atomic Member active-removal; depend on CRUD, block only active leadership that requires reassignment, end active Team membership, revoke Member access/profiles/sessions, and mark the Member inactive without deleting historical business data. Deactivate the linked User only when no other valid active identity remains. Test successful removal, active-leadership conflict, and retained alternate identity.
        VERIFY: grep -qE "teamMember\.updateMany|actorProfile\.updateMany|userRole\.updateMany|authSession\.updateMany|isActive: false" src/member/repositories/member-removal.repository.ts && corepack yarn test --runInBand --testPathPatterns=member-removal.repository.spec.ts
...
- [x] **Phase 3: authorization and error behavior**
  - [x] Apply Member authorization scopes; depend on routes, allow `system_admin` Member creation anywhere in own Company, `division_head` own-Company leadership/member scope, `division_lead` creation only in own Division, and `team_lead` ordinary Member creation only in exact led-Team scope; preserve configured `ADD_MEMBER`/object scope, keep update/delete/access-link on existing system_admin authority unless later contracts expand it, and retain 401/403/foreign-404 semantics. Test every actor and Tenant/Company/Division/Team direction; complete when guard and authorization tests pass.
        VERIFY: grep -q "ADD_MEMBER" src/member/member-api.decorator.ts && grep -qE "division_lead|leadMemberId|actorProfile|memberId|divisionId" src/member/providers/member-scope.provider.ts && corepack yarn test --runInBand --testPathPatterns=member
...
- [ ] **Phase 4: independent verification and sign-off**
  - [ ] Add focused tests for all CRUD/link/onboarding/identity-separation requirements; depend on implementation, prove `POST /member` creates linked User and Member without UserRole/Member-backed ActorProfile, access-link remains exceptional, no automatic Member from standalone User/UserRole, legacy unlinked Member validity, no automatic Team assignment from Member creation, active-removal access revocation, uniqueness, and authorization matrix. Complete only when independent tests exit zero.
