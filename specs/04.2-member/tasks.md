...
- [x] **Phase 1: Member CRUD foundation**
  ...
  - [x] Implement create/update business DTO allow-lists; depend on scoped reads, derive Tenant/Company, validate actor-scoped Division and Tenant-unique business email, reject security/User/Team fields, create no Team assignment, and block a Division move that conflicts with active Team lead/membership. Test a Member without User/Team, no persistence-level Team assignment, safe move, conflict move, and invalid input; complete when DTO/service tests pass.
...
- [x] **Phase 2: explicit User/ActorProfile linking**
  - [x] Implement only the existing-User access-link endpoint; depend on Member CRUD, prohibit User/password/session/role creation, link `userId` only after same-Tenant validation, and preserve the nullable unlinked state. Test User creation/role assignment produces no Member and foreign User concealment; complete when tests pass.
...
- [x] **Phase 3: authorization and error behavior**
  - [x] Apply Member authorization scopes; depend on routes, allow `system_admin` Member creation anywhere in own Company, `division_lead` creation only in own Division, contextual Team Lead creation only in the Division of the exact Team they lead, preserve configured `ADD_MEMBER`/object scope, keep update/delete/access-link on existing system_admin authority, and retain 401/403/foreign-404 semantics. Test every actor and Tenant/Company/Division direction; complete when guard and authorization tests pass.
        VERIFY: grep -q "ADD_MEMBER" src/member/member.controller.ts && grep -qE "division_lead|leadMemberId|actorProfile|memberId|divisionId" src/member/providers/member-scope.provider.ts && ! grep -rq "team_lead" src/member prisma/schema.prisma && corepack yarn test --runInBand --testPathPatterns=member
