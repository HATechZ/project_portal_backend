...
1. **Company types are global, companies are tenant-scoped.** `company_types` carries no
   `tenant_id` — it is reference data shared by every tenant, so the repository reads it through
   the unscoped client on purpose. `companies` carries `tenant_id` and is read through
   `prisma.scoped`. Mixing the two clients in one repository is deliberate and is the single
   thing a reviewer is most likely to misread as a tenant leak.
...
| # | Rule | Enforced by |
|---|---|---|
| DR-01 | A company always belongs to exactly one tenant | `prisma.scoped` + `@@unique([tenantId, abbr])` |
...
| Deviation | Rule |
|---|---|
| `CompanyRepository` injects `PrismaService` and does not extend `BaseRepository` — it cannot join an ambient transaction | X, VI.5 |
