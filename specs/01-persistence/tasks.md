...
- [ ] **Phase 1: Schema generation pipeline**
  - [ ] Keep the DBML as the schema authority
  ...
  - [x] Supply the CLI datasource from config
        VERIFY: grep -q "env('DATABASE_URL_MIGRATION')" prisma.config.ts
...
- [ ] **Phase 3: Unit of work**
  ...
  - [ ] Open bounded scoped transactions and set the bound tenant GUC before repository work
