...
| **01.1** | **Schema Integrity & Tenant Isolation** — RLS + privileged relay role, composite tenant-carrying FKs, identity split, derived-column removal, lookup collapse | — (reshapes 52) | **Phase 4** | 12/45 | `[WIP]` | P-1 closed. Phase 5 and pre-RLS Phase 6 application wiring verified. Phase 6 database execution/verification is **BLOCKED/DEFERRED solely because the separate administrative migration credential is unavailable**; its approved 53-table migration remains pending and unchanged. Phase 7 audit is in progress; no Phase 7–12 database execution is authorized. |
