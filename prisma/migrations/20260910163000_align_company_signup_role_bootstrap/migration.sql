-- Align company signup tenant bootstrapping with the current ActorRoleCode set.
-- Rollback: recreate provision_company_workspace_core with the previous role-count
-- guard and permission CASE expression if the leadership roles are rolled back.
...
BEGIN
  ...
  SELECT
    ...
    CASE role_definition.code
      ...
      WHEN 'division_head'::public.actor_role_code THEN action_definition.code::text = ANY (ARRAY[
        'ADD_DIVISION','ADD_TEAM','ASSIGN_LEADER'])
      WHEN 'division_lead'::public.actor_role_code THEN action_definition.code::text = ANY (ARRAY[
        'ADD_TEAM','REQUEST_INFO_FROM_MARKETING','PM_LEAD_RESPOND_TO_MEMBER','PM_RETURN_TO_MEMBER',
        'ASSIGN_MEMBER','FORWARD_TO_TMS','ORIGIN_MANAGER_APPROVE','FORWARD_TO_CCR',
        'ADD_WORK_REQUEST_DOCUMENT','ADD_WORK_REQUEST_NOTE','REQUEST_WORKFLOW_INFO','RESPOND_WORKFLOW_INFO'])
      WHEN 'team_lead'::public.actor_role_code THEN action_definition.code::text = ANY (ARRAY[
