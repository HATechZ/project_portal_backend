-- Restore unambiguous provisioning after the General Document Code catalogue
-- introduced conflict-target names that collide with this function's RETURNS TABLE
-- output variables. The underlying tenant/group/code and tenant/action/role unique
-- indexes remain the concurrency authority.
--
-- Rollback: recreate the immediately preceding function definition only if the
-- General Document Code and Designation provisioning additions are also rolled back.
-- This migration changes no provisioned data.
