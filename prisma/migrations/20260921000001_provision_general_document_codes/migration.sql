-- Provision General Document Codes without changing any existing tenant data.
-- The initial catalogue is ordinary tenant data: conflicts preserve existing rows.
--
-- Rollback: REVOKE SELECT, INSERT, UPDATE ON public.document_code_options FROM app_user;
-- delete this action's permission rows and definition only after ensuring no tenant
-- depends on it; rebuild the enum as documented in the preceding migration. Remove
-- provisioned code rows manually only after confirming they are unreferenced.
...
VALUES (
  gen_random_uuid(),
  'MANAGE_GENERAL_DOCUMENT_CODES'::public.workflow_action_code,
  'Manage General Document Codes',
  'Allows the manage general document codes action.',
  true, false, false, false, false
)
ON CONFLICT (code) DO NOTHING;

-- The permission matrix includes all system roles; only the approved three receive
-- an allowed grant for this action. Custom roles are intentionally excluded.
...
WHERE action_definition.code = 'MANAGE_GENERAL_DOCUMENT_CODES'::public.workflow_action_code
ON CONFLICT (tenant_id, action_id, role_id)
DO UPDATE SET allowed = EXCLUDED.allowed;

-- Backfill every current tenant with missing rows only. The existing unique key on
-- (tenant_id, document_group, code) makes this repeatable and concurrency-safe.
INSERT INTO public.document_code_options (
  tenant_id, id, document_group, code, name, description
)
SELECT
  tenant.id,
  gen_random_uuid(),
...
CROSS JOIN (
  VALUES
    ...
    ('002', 'Cargo Information'),
    ('011', 'Action Log'),
    ('012', 'Contact List'),
    ('013', 'Comment Sheet'),
    ('802', 'Contract'),
    ('803', 'Invoice')
) AS catalogue(code, name)
