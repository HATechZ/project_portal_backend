-- Restore company workspace provisioning after Bid/Project grant snippets added
-- ambiguous ON CONFLICT column targets to a PL/pgSQL function with a tenant_id
-- RETURNS TABLE output variable. This replaces only the function body; it changes
-- no existing tenant data.
--
-- Rollback: recreate the prior provision_company_workspace_core function only if
-- the Bid/Project provisioning additions are rolled back as well. The migration is
-- transactional, so a failed deployment leaves the prior function intact.
...
BEGIN
  ...
  SELECT sr.role_id INTO STRICT v_system_admin_role_id FROM public.system_roles AS sr
  ...
  ON CONFLICT DO NOTHING;
  UPDATE public.workflow_action_role_permissions AS permission_grant
  SET allowed = system_role.system_code IN ('system_admin'::public.actor_role_code, 'division_head'::public.actor_role_code)
  FROM public.system_roles AS system_role CROSS JOIN public.workflow_action_definitions AS action_definition
  WHERE permission_grant.tenant_id = v_tenant_id
    AND permission_grant.role_id = system_role.role_id
    AND permission_grant.action_id = action_definition.id
    AND action_definition.code = 'MANAGE_DESIGNATIONS'::public.workflow_action_code;

  -- Bid grants: add missing rows safely, then set the owner-locked policy.
  INSERT INTO public.workflow_action_role_permissions (tenant_id, id, action_id, role_id, allowed)
  SELECT v_tenant_id, gen_random_uuid(), action_definition.id, system_role.role_id,
    CASE action_definition.code
      WHEN 'VIEW_BID'::public.workflow_action_code THEN system_role.system_code IN ('system_admin'::public.actor_role_code, 'ccr_coordinator'::public.actor_role_code, 'division_head'::public.actor_role_code, 'division_lead'::public.actor_role_code)
      WHEN 'UPDATE_BID'::public.workflow_action_code THEN system_role.system_code IN ('system_admin'::public.actor_role_code, 'ccr_coordinator'::public.actor_role_code)
      ELSE false
    END
  FROM public.system_roles AS system_role CROSS JOIN public.workflow_action_definitions AS action_definition
  WHERE action_definition.code IN ('VIEW_BID'::public.workflow_action_code, 'UPDATE_BID'::public.workflow_action_code)
  ON CONFLICT DO NOTHING;
  UPDATE public.workflow_action_role_permissions AS permission_grant
  SET allowed = CASE action_definition.code
    WHEN 'VIEW_BID'::public.workflow_action_code THEN system_role.system_code IN ('system_admin'::public.actor_role_code, 'ccr_coordinator'::public.actor_role_code, 'division_head'::public.actor_role_code, 'division_lead'::public.actor_role_code)
    WHEN 'UPDATE_BID'::public.workflow_action_code THEN system_role.system_code IN ('system_admin'::public.actor_role_code, 'ccr_coordinator'::public.actor_role_code)
    ELSE false
  END
  ...
  WHERE permission_grant.tenant_id = v_tenant_id
    AND permission_grant.role_id = system_role.role_id
    AND permission_grant.action_id = action_definition.id
    AND action_definition.code IN ('VIEW_BID'::public.workflow_action_code, 'UPDATE_BID'::public.workflow_action_code);

  -- Project grants: ADD_PROJECT, VIEW_PROJECT, and UPDATE_PROJECT stay explicit
  ...
  WHERE permission_grant.tenant_id = v_tenant_id
    AND permission_grant.role_id = system_role.role_id
    AND permission_grant.action_id = action_definition.id
    AND action_definition.code IN ('ADD_PROJECT'::public.workflow_action_code, 'VIEW_PROJECT'::public.workflow_action_code, 'UPDATE_PROJECT'::public.workflow_action_code);

  IF (SELECT count(*) FROM public.workflow_action_role_permissions AS permission_grant
